...
def _preview(
    ...
    """
    Returns a preview of the comment so that the user may decide if he or she
    wants to edit it before submitting it permanently.
    """
...
<errorlist>
    ...
    {% endfor %}
        ...
        """
        response_str = Template(
            template_str,
        ).render(Context({
            'errors': zip(form.errors.values(), form.errors.keys())}),
        )
...
def comment(*args, **kwargs):
    """
    Thin wrapper around free_comment which adds login_required status and also
    assigns the ``model`` to be ``ThreadedComment``.
    """
...
def comment_delete(
    ...
    ``FreeThreadedComment`` or a ``ThreadedComment``.  If it is a POST request,
    then the comment will be deleted outright, however, if it is a GET request,
